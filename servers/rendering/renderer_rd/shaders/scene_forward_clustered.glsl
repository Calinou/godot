	...
	m_var.xyz += normal_bias;
				...
				if (sc_use_directional_soft_shadows && directional_lights.data[i].softshadow_angle > 0) {
					...
					if (depth_z < directional_lights.data[i].shadow_split_offsets.x) {
						...
						shadow = sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[i].soft_shadow_scale, scene_data.taa_jitter);
						blend_count++;
					}
					...
					if (blend_count < blend_max && depth_z < directional_lights.data[i].shadow_split_offsets.y) {
						...
						float s = sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[i].soft_shadow_scale, scene_data.taa_jitter);
					...
					shadow = sample_directional_pcf_shadow(directional_shadow_atlas, scene_data.directional_shadow_pixel_size * directional_lights.data[i].soft_shadow_scale * blur_factor, pssm_coord, scene_data.taa_jitter);
					...
					if (directional_lights.data[i].blend_splits) {
						...
						float shadow2 = sample_directional_pcf_shadow(directional_shadow_atlas, scene_data.directional_shadow_pixel_size * directional_lights.data[i].soft_shadow_scale * blur_factor2, pssm_coord, scene_data.taa_jitter);
	...
	{ //omni lights
		...
		for (uint i = item_from; i < item_to; i++) {
			...
			while (merged_mask != 0) {
				...
				float shadow = light_process_omni_shadow(light_index, vertex, normal, scene_data.taa_jitter);
	...
	{ //spot lights
		...
		for (uint i = item_from; i < item_to; i++) {
			...
			while (merged_mask != 0) {
				...
				float shadow = light_process_spot_shadow(light_index, vertex, normal, scene_data.taa_jitter);
