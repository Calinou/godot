	...
	m_var.xyz += normal_bias;
				...
				if (depth_z < directional_lights.data[i].shadow_split_offsets.x) {
					...
					if (directional_lights.data[i].softshadow_angle > 0) {
						...
						shadow = sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[i].soft_shadow_scale, vec2(0.0));
					} else {
						shadow = sample_directional_pcf_shadow(directional_shadow_atlas, scene_data.directional_shadow_pixel_size * directional_lights.data[i].soft_shadow_scale, pssm_coord, vec2(0.0));
					}
				...
				if (directional_lights.data[i].blend_splits) {
					...
					if (depth_z < directional_lights.data[i].shadow_split_offsets.x) {
						...
						if (directional_lights.data[i].softshadow_angle > 0) {
							...
							float test_radius = (range_pos - range_begin) * directional_lights.data[i].softshadow_angle;
							vec2 tex_scale = directional_lights.data[i].uv_scale2 * test_radius;
							shadow2 = sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[i].soft_shadow_scale, vec2(0.0));
						} else {
							shadow2 = sample_directional_pcf_shadow(directional_shadow_atlas, scene_data.directional_shadow_pixel_size * directional_lights.data[i].soft_shadow_scale, pssm_coord, vec2(0.0));
						}
...
#define BIAS_FUNC(m_var, m_idx)                                                                 \
	...
	m_var.xyz += normal_bias;
				...
				shadow = sample_directional_pcf_shadow(directional_shadow_atlas, scene_data.directional_shadow_pixel_size * directional_lights.data[i].soft_shadow_scale * blur_factor, pssm_coord, vec2(0.0));
				...
				if (directional_lights.data[i].blend_splits) {
					...
					float shadow2 = sample_directional_pcf_shadow(directional_shadow_atlas, scene_data.directional_shadow_pixel_size * directional_lights.data[i].soft_shadow_scale * blur_factor2, pssm_coord, vec2(0.0));
					shadow = mix(shadow, shadow2, pssm_blend);
				}
...
*/
	...
	if (!sc_disable_omni_lights) { //omni lights
		...
		for (uint i = 0; i < 8; i++) {
			...
			float shadow = light_process_omni_shadow(light_index, vertex, normal, vec2(0.0));
...
*/
	...
	if (!sc_disable_spot_lights) { //spot lights
		...
		for (uint i = 0; i < 8; i++) {
			...
			float shadow = light_process_spot_shadow(light_index, vertex, normal, vec2(0.0));
