...
// If alpha scissor is used, we must further the edge threshold, otherwise we won't get any edge feather
	...
	if (bool(instances.data[draw_call.instance_index].flags & INSTANCE_FLAGS_USE_LIGHTMAP_CAPTURE)) { //has lightmap capture
		...
		if (uses_sh) {
			...
			sh_light += lm_light_l1n1 * n.y * (lm_light_l0 * exposure_normalization * 4.0);
			sh_light += lm_light_l1_0 * n.z * (lm_light_l0 * exposure_normalization * 4.0);
			sh_light += lm_light_l1p1 * n.x * (lm_light_l0 * exposure_normalization * 4.0);
			...
			light_compute(normal, L, view, 0.0, sh_light, false, 1.0, f0, orms, 50.0 /* Boosted specular amount for testing */, albedo, alpha, screen_uv,
