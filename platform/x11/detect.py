...
def get_opts():
    ...
    return [
        ...
        BoolVariable('use_thinlto', 'Enable lightweight link-time optimization using ThinLTO', False),
...
def configure(env):
    ...
    if env['use_thinlto']:
        if env['use_llvm']:
            # Always enable LTO when ThinLTO is enabled
            env['use_lto'] = True
        else:
            print("Using ThinLTO with GCC is not supported yet, try compiling with 'use_llvm=yes'.")
            sys.exit(255)
