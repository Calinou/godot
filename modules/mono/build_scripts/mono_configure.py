...
def get_android_out_dir(env):
    return os.path.join(
        Dir('#platform/android/java/libs').abspath,
    ...
    if env['platform'] == 'windows':
        ...
        if mono_static:
            ...
            if not os.path.isfile(
                    os.path.join(mono_lib_path, mono_static_lib_name + lib_suffix)):
            ...
            if env.msvc:
                ...
                env.Append(LINKFLAGS=os.path.join(mono_lib_path, mono_static_lib_name +
                                                  lib_suffix))
            ...
            if mono_static:
                ...
                if is_apple:
                    ...
                    env.Append(LINKFLAGS=[
                        '-Wl,-whole-archive', mono_lib_file, '-Wl,-no-whole-archive'
                    ])
            else:
    ...
    if not tools_enabled and not is_android:
        if not mono_root:
            mono_root = subprocess.check_output(
                ['pkg-config', 'mono-2', '--variable=prefix']).decode('utf8').strip()
        ...
        make_android_mono_config.generate_compressed_config(config_file_path,
                                                            'mono_gd/')
...
def copy_mono_root_files(env, mono_root):
    ...
    editor_mono_framework_facades_dir = os.path.join(editor_mono_framework_dir,
                                                     'Facades')
...
def copy_mono_etc_dir(mono_root, target_mono_config_dir, platform):
    ...
    copy_tree(os.path.join(mono_etc_dir, '2.0'),
              os.path.join(target_mono_config_dir, '2.0'))
    copy_tree(os.path.join(mono_etc_dir, '4.0'),
              os.path.join(target_mono_config_dir, '4.0'))
    copy_tree(os.path.join(mono_etc_dir, '4.5'),
              os.path.join(target_mono_config_dir, '4.5'))
...
def copy_mono_shared_libs(env, mono_root, target_mono_root_dir):
    ...
    if platform == 'windows':
        ...
        target_mono_lib_dir = get_android_out_dir(
        ...
        if platform == 'osx':
            ...
            copy(os.path.join(mono_root, 'lib', 'libMonoPosixHelper.dylib'),
                 target_mono_lib_dir)
        elif platform == 'x11' or platform == 'android':
            lib_file_names = [
                lib_name + '.so' for lib_name in [
                    'libmono-btls-shared', 'libmono-ee-interp', 'libmono-native',
                    'libMonoPosixHelper', 'libmono-profiler-aot',
                    'libmono-profiler-coverage', 'libmono-profiler-log',
                    'libMonoSupportW'
                ]
            ]

            for lib_file_name in lib_file_names:
                copy_if_exists(os.path.join(mono_root, 'lib', lib_file_name),
                               target_mono_lib_dir)
